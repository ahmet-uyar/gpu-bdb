...
# INTERFACE=${INTERFACE:-enp129s0f1}

# Logging and scratch space for your machine or cluster
# Repository home
##############################  MODIFY ##################
GPU_BDB_HOME=${GPU_BDB_HOME:-/path-to-project-dir/gpu-bdb}

LOCAL_DIRECTORY=${LOCAL_DIRECTORY:-$GPU_BDB_HOME/dask-local-directory}
SCHEDULER_FILE=${SCHEDULER_FILE:-$LOCAL_DIRECTORY/scheduler.json}
LOGDIR=${LOGDIR:-$LOCAL_DIRECTORY/logs}
WORKER_DIR=${WORKER_DIR:-$GPU_BDB_HOME/gpu-bdb-dask-workers/}

# Communication protocol
############################## MAYBE MODIFY ##################
# CLUSTER_MODE=${CLUSTER_MODE:-TCP}
CLUSTER_MODE=${CLUSTER_MODE:-NVLINK}
...
CONDA_ENV_NAME=${CONDA_ENV_NAME:-gpu_bdb_dev}
...
if [ "$ROLE" = "SCHEDULER" ]; then
  if [ "$CLUSTER_MODE" = "NVLINK" ]; then
     ...
     nohup dask-scheduler --dashboard-address $DASHBOARD_ADDRESS \
                          --port $SCHEDULER_PORT \
                          --interface $INTERFACE \
                          --protocol ucx \
...
if [ "$CLUSTER_MODE" = "TCP" ]; then
    echo "Starting workers with TCP ..."
    dask-cuda-worker --device-memory-limit $DEVICE_MEMORY_LIMIT \
                     --local-directory $WORKER_DIR  \
                     --rmm-pool-size $POOL_SIZE \
                     --memory-limit=$MAX_SYSTEM_MEMORY \
